...
class Yahoo extends GroupModule {
...
//        echo $data['data'];
    ...
    public function check($group) {
        ...
        foreach ($group['keywords'] as $keyKW => $keyword) {
            ...
            $pos=1;
            $start_index=0;
            if(!empty($group['options']['local'])){$lang = '&rd=pref&lang='.$group['options']['local'];} else {$lang = "";}
            // init a new session
            ...
            do{
                ...
                if($start_index==0){
                    $url="https://search.$domain/search?p=".urlencode($keyword).$lang;
                    $referrer= "https://search.$domain/";                    
                }else{
                    $referrer=$url;
                    $url="https://search.$domain/search?p=".urlencode($keyword).$lang."&start_index=".($start_index);
                }
...
//                                $redir = str_replace("://www.google.com/sorry/?continue=", "://".$domain."/sorry/?continue=", $redir);
                                ...
                                if($data['status'] == 403){
                                    $proxies->ban($proxy);
                                    $this->w("IP banned from Yahoo (no captcha), force proxy remove");
                                    $error = true;
                                }else{
                                    $this->w("Yahoo captcha");
                                    if($dbc == null){
