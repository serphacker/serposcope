...
class Bing extends GroupModule {
    ...
    public function getGlobalOptions() {
        return array(
            array(
                'page_sleep',
                '30',
                'pause in seconds between request to bing',
                '/^[0-9]+$/',
                'text'
            ),
            array(
    ...
    public function getGroupOptions() {
        ...
        return array(
            array(
                'parameters',
                '',
                'Dodatkowe parametry w wywołaniu',
                '/^.*$/',
                'text'
            ),
        );
        ...
        foreach ($group['keywords'] as $keyKW => $keyword) {
            ...
            do{
                ...
                if($start_index==0){
                    ...
                    $referrer=$url;
                    $url="https://$domain/search?q=".urlencode($keyword)."&first=".($start_index);
                }
...
//                            file_put_contents("/tmp/noserp_". sha1("".time().rand(0, 10000)), $data);
                ...
                $allh3 = $doc->getElementsByTagName("h2");
               
                foreach($allh3 as $h3){
                    if(!$h3->hasAttribute("style") && !$h3->getAttribute("class")){
                        try {
