...
function getUserOptions() {
	return array(
            array('setting','0','Edycja ustawień','Aktywuje dostęp do edycji<br />ustawień projektu'),
            array('info','1','Edycja opisu','Aktywuje dostęp do edycji opisów'),
            array('events','1','Edycja notatek','Aktywuje dostęp do edycji notatek '),
            array('del','0','Przycisk Usuń','Umożliwia usunięcie wybranego projektu'),
            array('restart','0','Przycisk Uruchom','Aktywuje dostęp do ręcznego startu oraz usuwania wybranych danych'),
            array('export','1','Przycisk Export','Aktywuje dostęp do zmian widoku'),
			);
}
$group = null;
$keywords = array();
    ...
    <form method="POST" class="well form-horizontal" action="edit.php?idGroup=<?php echo intval($_GET['idGroup']); ?>" >
        <fieldset>
            ...
            <div class="control-group">
                <label class="control-label" for="keywords">Słowa kluczowe</label>
            ...
            <div id="useropt"><center>
                ...
                // echo h8($group['user']->setting).'<br/>';  //debug
                 foreach (getUserOptions() as $options) {
                 	...
                 	if (!$adminAcces && !$userAcces){$helps ='rel="tooltip" title ="Nie posiadasz wystarczających<br />uprawnień do zmiany tych opcji" class="disabled"';}
                    echo '<div class="control-group" >';
